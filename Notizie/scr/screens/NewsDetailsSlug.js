...
const NewsDetailsSlug = ({route, navigation}) => {
  ...
  const ResultData = async () => {
    try {
      const Data = await serverRequest('/wp-json/wp/v2/posts?slug=' + id);
      console.log('ID: ' + id);
      await Data.slice(0, 1).map(async (item, key2) => {
        console.log(item.title.rendered);
        setNameData(entities.decode(item.title.rendered));
  ...
  const checkOnPressEvent = (event, href, slug) => {
    try {
      ...
      if (dataCheck === true) {
        ...
        navigation2.push('NewsDetailsSlug', {
          name: finalData,
          id: slugData,
          link: href,
        });
    ...
    <View style={styles.container}>
      {loading ? (
        ...
        <ScrollView style={{paddingVertical: 15, paddingHorizontal: 15}}>
          ...
          <Text style={styles.text}>{NameData}</Text>
          ...
          <View style={{flexDirection: 'row'}}>
            ...
            <Text style={styles.textDateR}>{authors}</Text>
          </View>
...
export default NewsDetailsSlug;
