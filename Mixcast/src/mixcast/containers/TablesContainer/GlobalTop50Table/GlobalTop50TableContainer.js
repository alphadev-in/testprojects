...
import {
  addGlobalTop50Playlist,
  getGlobalTop50Playlist
} from '../../../../reducers/SpotifyGlobalTopState';
...
const TablesContainer = (props) => {
  ...
  useEffect(() => {
    if (status === MixcastLoginState.LOGGED) {
      getGlobalTop50Playlist().then((result) => {
        addGlobalTop50Playlist(result);
      });
    }
  ...
  useEffect(() => {
    if (spotifyGlobalTopTracks.length) {
      ...
      setNumberOfPages(
        Math.round(spotifyGlobalTopTracks.length / NUMBER_OF_TRACKS)
      );
  ...
  }, [spotifyGlobalTopTracks, page]);

  function onPlaySongHandler(trackId, type) {
    if (type === 'play') {
      const trackToPlay = spotifyGlobalTopTracks.find(
...
const mapStateToProps = (state) => ({
  ...
  spotifyGlobalTopTracks: state.SpotifyGlobalTopState.items,
  followedArtists: state.MixcastState.followingArtists
});

const mapDispatchToProps = (dispatch) => ({
  addGlobalTop50Playlist: (tracks) => dispatch(addGlobalTop50Playlist(tracks)),
